...
function get_selected_appr_krs() {
	...
	// } else {
		...
		$.get({
			url: '/approve-krs-koprodi/get_krs_mhs_all',
			data: {
				angkatan: angkatan,
				program: program,
			...
			success: function (data) {
				...
				$.each(data.mhs, function (key, val) {
					...
					var nim = val.nim;
					link = "/approve-krs-koprodi/lihat?nim=" + nim + "&tahun_ajaran=" + tahun_ajaran + "&semester=" + semester;
					...
					if (val.has_krs_count > 0) {
                        ...
                        if (val.krs_approve_count == val.has_krs_count) {   
                            btn += '<button class="btn btn-primary btn-sm m-1" onclick="approve_all_koprodi(' + val['nim'] + ')" data-toggle="tooltip" title="Approve Semua Matakuliah"><span class="mdi mdi-check-circle"></span></button>';
                        }						
					}
				...
				$('#table-approve').DataTable().destroy();
...
function approve_all_koprodi(nim) {
	...
	swal({
		...
		$.post({
			url: '/approve-krs-koprodi/approve_all',
			data: {
				_token: token,
				tahun_ajaran: tahun_ajaran,
				semester: semester,
